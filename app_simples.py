...
def main():
    ...
    try:
        ...
        #  imagems
        imagem_atual = None
        imagem_processada1 = None
        imagem_processada2 = None
        
        #  janela única para mostrar tudo
        cv2.namedWindow('CARLA', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('CARLA', LARGURA*2, ALTURA)
        
 
        # Função para processar a imagem da câmara
        def processar_imagem(imagem):
            nonlocal imagem_atual, imagem_processada1, imagem_processada2
            ...
            if imagem_atual is not None:
                ...
                cinza = cv2.cvtColor(imagem_atual, cv2.COLOR_RGB2BGR)
                
                # Primeiro processamento: deteção de bordas
                desfoque = cv2.GaussianBlur(cinza, (5, 5), 0)
                edges = cv2.Canny(desfoque, 50, 150)
                # Converter de volta para BGR para conseguir combinar as imagens
                imagem_processada1 = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
                
                # Segundo processamento: deteção de linhas em amarelo/branco (para faixas)
                hsv = cv2.cvtColor(cinza, cv2.COLOR_BGR2HSV)
                ...
                imagem_processada2 = cv2.bitwise_and(cinza, cinza, mask=mask_combined)
        ...
        while a_correr:
            ...
            # Mostrar as imagens
            if imagem_atual is not None:
                # Criar tela vazia para a grelha
                tela = np.zeros((ALTURA, LARGURA*2, 3), dtype=np.uint8)
                
                # Converter imagem original para BGR 
                img_original = cv2.cvtColor(imagem_atual, cv2.COLOR_RGB2BGR)
                ...
                processada1 = imagem_processada1 if imagem_processada1 is not None else np.zeros_like(img_original)
          
                
              
                cv2.putText(processada1, "Edges", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                
                tela[0:ALTURA, 0:LARGURA] = img_original
                tela[0:ALTURA, LARGURA:LARGURA*2] = processada1
                
                cv2.imshow('CARLA', tela)
