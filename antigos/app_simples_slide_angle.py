...
def detetar_linhas_estrada(imagem, params):
    ...
    if linhas is not None:
        for linha in linhas:
            ...
            if inclinacao < 0 and x1_float < centro_x:
                ...
                # Adicionar ambos os pontos da linha
                pontos_esquerda_x.extend([x1, x2])
                pontos_esquerda_y.extend([y1, y2])
                ...
                # Adicionar ambos os pontos da linha
                pontos_direita_x.extend([x1, x2])
                pontos_direita_y.extend([y1, y2])
    
    # Calcular ângulos das linhas
    angulo_esquerda = None
    angulo_direita = None
    
    try:
        # Se tivermos pontos suficientes, ajustar polinómios
        if len(pontos_esquerda_x) > 3:
            # Ajustar polinómio à linha esquerda
            z_esq = np.polyfit(pontos_esquerda_y, pontos_esquerda_x, 2)
            # Criar função para o polinómio
            f_esquerda = np.poly1d(z_esq)
            
            # Calcular ângulo tangente na parte inferior da imagem
            y_avaliacao = altura  # Base da imagem
            # Derivada do polinómio no ponto de avaliação (dá-nos o declive)
            declive = 2 * z_esq[0] * y_avaliacao + z_esq[1]
            angulo_esquerda = np.arctan(declive) * 180 / np.pi
            
            # Desenhar polinómio ajustado
            y_pontos = np.linspace(altura//2, altura, 20)
            x_pontos = f_esquerda(y_pontos)
            pontos = np.column_stack((x_pontos.astype(np.int32), y_pontos.astype(np.int32)))
            cv2.polylines(imagem_linhas, [pontos], False, (0, 255, 255), 4)
        
        if len(pontos_direita_x) > 3:
            # Ajustar polinómio à linha direita
            z_dir = np.polyfit(pontos_direita_y, pontos_direita_x, 2)
            # Criar função para o polinómio
            f_direita = np.poly1d(z_dir)
            
            # Calcular ângulo tangente na parte inferior da imagem
            y_avaliacao = altura  # Base da imagem
            # Derivada do polinómio no ponto de avaliação
            declive = 2 * z_dir[0] * y_avaliacao + z_dir[1]
            angulo_direita = np.arctan(declive) * 180 / np.pi
            
            # Desenhar polinómio ajustado
            y_pontos = np.linspace(altura//2, altura, 20)
            x_pontos = f_direita(y_pontos)
            pontos = np.column_stack((x_pontos.astype(np.int32), y_pontos.astype(np.int32)))
            cv2.polylines(imagem_linhas, [pontos], False, (0, 255, 255), 4)
    ...
    # Adicionar informação de ângulo à imagem
    if angulo_esquerda is not None:
        cv2.putText(imagem_linhas, f"Angulo Esq: {angulo_esquerda:.1f}°", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    
    if angulo_direita is not None:
        cv2.putText(imagem_linhas, f"Angulo Dir: {angulo_direita:.1f}°", 
                   (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    
    # Combinar as imagens
    resultado = cv2.addWeighted(imagem, 0.8, imagem_linhas, 1, 0)
    
    return resultado, bordas_coloridas, binario, imagem_linhas, angulo_esquerda, angulo_direita
    ...
    try:
        ...
        while a_correr:
            ...
            if imagem_atual is not None:
                ...
                resultado, bordas, binario, linhas,angulo_esquerda, angulo_direita = detetar_linhas_estrada(imagem_bgr, params)
